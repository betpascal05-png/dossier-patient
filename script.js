...
document.getElementById('login-btn').onclick = async () => {
    ...
    catch(e) { document.getElementById('login-error').innerText = "Erreur d'accès"; }
};
    ...
    filtered.forEach(p => {
        ...
        list.innerHTML += `
            <tr>
                ...
                <td>${p.auteur ? p.auteur.split('@')[0] : 'Admin'}</td>
                <td>
...
window.deleteP = async (id) => { if(confirm("Supprimer ce dossier ?")) await deleteDoc(doc(db, "patients", id)); };
